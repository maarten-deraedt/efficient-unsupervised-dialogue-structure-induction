...
def evaluate(clusterer, n_system, n_user):
   metrics_user = clusterer.cluster_eval(n_user, user=True)
   metrics_system = clusterer.cluster_eval(n_system, user=False)
...
for task, service in zip(tasks, services):
    ...
    out_file = f"results/{task}/{service.lower()}-{embedder_name}.json" if service is not None else f"ablations/{task}/{embedder_name}.json"
    ...
    for run in range(num_runs):
        ...
        metrics_user_c, metrics_system_c = evaluate(c, n_system=n_system, n_user=n_user)
        metrics_user_pc, metrics_system_pc = evaluate(pc, n_system=n_system, n_user=n_user)
        metrics_user_pcn, metrics_system_pcn = evaluate(pcn, n_system=n_system, n_user=n_user)

        metrics_user_int, metrics_system_int = evaluate(interpolator, n_system=n_system, n_user=n_user)
        metrics_user_ext, metrics_system_ext = evaluate(extrapolator, n_system=n_system, n_user=n_user)
        metrics_user_int_ext, metrics_system_int_ext = evaluate(hybrid, n_system=n_system, n_user=n_user)


        all_metrics["c"].append(combine_metrics(metrics_user_c, metrics_system_c))
        all_metrics["pc"].append(combine_metrics(metrics_user_pc, metrics_system_pc))
        all_metrics["pcn"].append(combine_metrics(metrics_user_pcn, metrics_system_pcn))

        all_metrics["int"].append(combine_metrics(metrics_user_int, metrics_system_int))
        all_metrics["ext"].append(combine_metrics(metrics_user_ext, metrics_system_ext))
        all_metrics["int_ext"].append(combine_metrics(metrics_user_int_ext, metrics_system_ext))
